...
class ApiClient {
	...
	async joinQueue(showId: string) {
		const response = await this.client.post('/queue/join', { showId });
		return response.data;
	}
	...
	async releaseSeat(showId: string, seatId: number) {
		const response = await this.client.post('/seats/release', {
			showId,
			seatId,
		});
		return response.data;
	...
	async login(credentials: { email: string; password: string }) {
		// withCredentials: true 덕분에 Set-Cookie 자동 저장
