...
export function useShows(params?: {
	status?: string;
	limit?: number;
	offset?: number;
}) {
	return useQuery({
		...
		queryFn: async () => {
			const response: ApiResponse<ShowListResponse> =
				await apiClient.getShows(params);
...
export function useSeats(showId: string | null) {
	return useQuery({
		...
		queryFn: async () => {
			if (!showId) throw new Error('Show ID is required');
			const response: ApiResponse<SeatListResponse> =
				await apiClient.getSeats(showId);
			return response.data;
		},
		enabled: !!showId,
...
export function useAvailableCount(showId: string | null) {
	return useQuery({
		...
		queryFn: async () => {
			if (!showId) throw new Error('Show ID is required');
			const response: ApiResponse<AvailableCountResponse> =
				await apiClient.getAvailableCount(showId);
			return response.data;
		},
		enabled: !!showId,
		staleTime: 3 * 1000, // 3초
