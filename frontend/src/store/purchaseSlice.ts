...
export const holdSeat = createAsyncThunk(
	'purchase/holdSeat',
	async (
		{ showId, seatId }: { showId: string; seatId: number },
		{ rejectWithValue },
	) => {
		try {
			const response: ApiResponse<HoldSeatResponse> = await apiClient.holdSeat(
				showId,
				seatId,
			);
	...
	async (data: any, { rejectWithValue }) => {
		try {
			const response: ApiResponse<PurchaseResponse> =
				await apiClient.purchaseNonReserved(data);
			return response.data;
		} catch (error: any) {
			return rejectWithValue(error);
		}
...
export const purchaseReserved = createAsyncThunk(
	...
	async (data: any, { rejectWithValue }) => {
		try {
			const response: ApiResponse<PurchaseResponse> =
				await apiClient.purchaseReserved(data);
			return response.data;
		} catch (error: any) {
			return rejectWithValue(error);
		}
...
const purchaseSlice = createSlice({
	...
	reducers: {
		...
		setQuantity: (state, action: PayloadAction<number>) => {
			...
			if (
				state.selectedShow &&
				state.selectedShow.seatType === 'non_reserved'
			) {
				const price =
					state.selectedShow.price.min ?? state.selectedShow.price.max ?? 0;
