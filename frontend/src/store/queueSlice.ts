...
export const joinQueue = createAsyncThunk(
	'queue/join',
	async ({ showId }: { showId: string }) => {
		const response: ApiResponse<QueueJoinResponse> =
			await apiClient.joinQueue(showId);
	...
	async (_, { rejectWithValue }) => {
		try {
			const response: ApiResponse<QueueStatusResponse> =
				await apiClient.getQueueStatus();
...
const queueSlice = createSlice({
	...
	reducers: {
		updatePosition: (
			state,
			action: PayloadAction<{ position: number; estimatedWaitTime: number }>,
		) => {
		...
		setAllowed: (
			state,
			action: PayloadAction<{ queueToken: string; allowedUntil: number }>,
		) => {
