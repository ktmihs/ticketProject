...
function PurchasePage() {
  ...
  const { queueToken, allowedUntil, status } = useAppSelector((state) => state.queue);
  ...
  // 구매 가능 시간 타이머
  useEffect(() => {
    const until = allowedUntil || parseInt(localStorage.getItem('allowedUntil') || '0');
    if (!until) return;
  ...
  const handleSeatSelect = async (seat: Seat) => {
    ...
    try {
      const result = await dispatch(holdSeat({ showId, seatId: seat.id })).unwrap();
    ...
    try {
      if (selectedShow.seatType === 'reserved' && holdToken) {
        await dispatch(
          purchaseReserved({
            holdToken,
            payment: { cardNumber: '1234-5678-9012-3456', expiryDate: '12/28', cvv: '123', cardHolder: 'Test User' },
          })
        ).unwrap();
      } else {
        await dispatch(
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 px-4 shadow-lg sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className={`text-3xl font-bold ${queueRemainingSeconds < 60 ? 'animate-pulse' : ''}`}>
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <p className="text-gray-500 mb-8">{new Date(selectedShow.date).toLocaleString('ko-KR')}</p>
        ...
        {selectedShow.seatType === 'reserved' && (
          <div className="bg-white rounded-xl shadow p-6 mb-6">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {Object.entries(gradeColors).map(([grade, cls]) => (
                <div key={grade} className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold ${cls}`}>{grade}</div>
                  <span className="text-sm text-gray-600">
                    {grade === 'VIP' ? '150,000원' : grade === 'R' ? '120,000원' : grade === 'S' ? '80,000원' : '50,000원'}
                  </span>
                </div>
              ))}
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-yellow-300 text-xs flex items-center justify-center">H</div>
                <span className="text-sm text-gray-500">선점중</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-gray-300 text-xs flex items-center justify-center">X</div>
            ...
            {seatsLoading ? (
              <div className="text-center py-8 text-gray-500">좌석 정보를 불러오는 중...</div>
            ) : seatData?.seats ? (
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <div key={row} className="flex items-center gap-1 mb-1">
                        <span className="w-6 text-xs text-gray-500 text-right font-mono">{row}</span>
                        <div className="flex gap-0.5 flex-wrap">
                          {rowSeats.map((seat: Seat) => {
                            ...
                            let cls = 'w-5 h-5 text-[8px] rounded flex items-center justify-center font-bold cursor-pointer transition-all ';
                            if (isSold) cls += gradeBgSold;
                            else if (isHolding) cls += gradeBgHolding;
                            else cls += (gradeColors[seat.grade] || 'bg-gray-400 text-white hover:bg-gray-500 ');
              ...
              <p className="text-gray-500 text-center py-4">좌석 정보를 불러올 수 없습니다.</p>
            )}
              ...
              <div className="mt-6 bg-green-50 border-2 border-green-200 rounded-lg p-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{selectedSeat.row}열 {selectedSeat.number}번</p>
                    <p className="text-sm text-gray-500">{selectedSeat.grade}석 · {selectedSeat.price?.toLocaleString()}원</p>
                    {holdExpiresAt && (
                      <p className={`text-sm mt-1 font-semibold ${remainingSeconds < 60 ? 'text-red-600 animate-pulse' : 'text-orange-600'}`}>
        ...
        {selectedShow.seatType === 'non_reserved' && (
          <div className="bg-white rounded-xl shadow p-6 mb-6">
            ...
            <p className="mt-4 text-lg font-semibold text-blue-600">
              총 금액: {((selectedShow.price.min ?? selectedShow.price.max ?? 0) * quantity).toLocaleString()}원
            </p>
          </div>
        )}
        ...
        {(selectedShow.seatType === 'non_reserved' || selectedSeat) && (
          ...
          >
            {isProcessing ? (
              ...
              `결제하기 (${selectedShow.seatType === 'reserved'
                ? (selectedSeat?.price ?? 0).toLocaleString()
                : ((selectedShow.price.min ?? 0) * quantity).toLocaleString()}원)`
            )}
          </button>
        )}
