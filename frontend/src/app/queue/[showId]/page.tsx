...
export default function QueuePage() {
  ...
  const connectSSE = (token: string) => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.status === 'WAITING') {
          ...
          // ✅ allowedUntil + 새 queueToken을 Redux + localStorage에 저장
          dispatch(
            setAllowed({
    ...
    eventSource.onerror = () => {
      ...
      if (reconnectRef.current < 5) {
        ...
        dispatch(setError('서버와의 연결이 끊어졌습니다. 페이지를 새로고침해주세요.'));
      }
    };
  };
  ...
  useEffect(() => {
    if (!isClient || status !== 'WAITING') return;
    const handler = () => {
      const ok = window.confirm('대기열에서 나가시겠습니까?\n순번이 초기화됩니다.');
      if (ok) {
        dispatch(resetQueue());
        router.replace('/');
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <button
            onClick={() => { dispatch(resetQueue()); router.push('/'); }}
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-6xl font-bold text-blue-600">{position ?? '...'}</p>
            <p className="text-sm text-gray-500 mt-1">번째</p>
          </div>

          <div className="text-center mb-6">
            <p className="text-sm text-gray-600 mb-2">예상 대기 시간</p>
            <p className="text-2xl font-semibold">{formatTime(estimatedWaitTime)}</p>
          </div>
