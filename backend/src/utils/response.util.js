...
const Errors = {
	...
	QUEUE_NOT_FOUND: () =>
		new ErrorResponse(
			'QUEUE_NOT_FOUND',
			'대기열 정보를 찾을 수 없습니다. 다시 진입해주세요',
			404,
		),
	ALREADY_IN_QUEUE: position =>
		new ErrorResponse('ALREADY_IN_QUEUE', '이미 대기열에 진입했습니다', 409, {
			currentPosition: position,
		}),
	QUEUE_NOT_ALLOWED: (currentStatus, position) =>
		new ErrorResponse('QUEUE_NOT_ALLOWED', '대기 후 이용 가능합니다', 403, {
			currentStatus,
			position,
		}),
	QUEUE_BLOCKED: () =>
	...
	SHOW_NOT_FOUND: showId =>
		new ErrorResponse('SHOW_NOT_FOUND', '존재하지 않는 공연입니다', 404, {
			showId,
		}),
	SEAT_NOT_FOUND: seatId =>
		new ErrorResponse('SEAT_NOT_FOUND', '존재하지 않는 좌석입니다', 404, {
			seatId,
		}),
	SEAT_ALREADY_HOLDING: (seatId, holdUntil) =>
		new ErrorResponse(
			'SEAT_ALREADY_HOLDING',
			'다른 사용자가 이미 선점한 좌석입니다',
			409,
			{ seatId, holdUntil },
		),
	SEAT_SOLD: seatId =>
	...
	HOLD_EXPIRED: expiredAt =>
		new ErrorResponse(
			'HOLD_EXPIRED',
			'좌석 선점 시간이 만료되었습니다. 다시 선택해주세요',
			410,
			{ expiredAt },
		),
	PAYMENT_FAILED: pgResponse =>
		new ErrorResponse('PAYMENT_FAILED', '결제에 실패했습니다', 422, {
			pgResponse,
		}),
